...
class NeuralNetworkProcess(multiprocessing.Process):
    ...
    def __init__(self, input_queue, output_queues, model=None):
        ...
        self.list_uid = []
        self.list_states = []
        self.batch_size = int(config.processes * config.threads) # number of states to process in a single batch
    ...
    def run(self):
        ...
        while True:
            ...
            request_type, tuid, uid, data = self.input_queue.get()
    ...
    def _process_requests(self):
        ...
        for input_repr, tuid, key, (p, v) in zip(model_input,self.list_tuid ,self.list_uid, result_zip):
            self.evaluations[input_repr.data.tobytes()] = (p, v)
            out_queue = self.output_queues[key]
            out_queue.put((tuid, p, v))
